...
public class BtcTurkClient : BaseRestClient
{
    ...
    {
        var result = await SingleApi.ExecuteAsync<BtcTurkApiResponse<object>>(SingleApi.GetUri(V1_Order_Endpoint + "?id=" + orderId.ToString(CultureInfo.InvariantCulture), V1), method: HttpMethod.Delete, ct, signed: true).ConfigureAwait(false);
    ...
    {
        var parameters = new Dictionary<string, object>();
        parameters.AddOptionalParameter("pairSymbol", pairSymbol ?? null);
    ...
    => await GetPriceGraphHistoryCoreAsync(PriceGraph_History_Endpoint, symbol, period, startTime, endTime, ct);
...
public class BtcTurkClientSingleApi : RestApiClient
{
    ...
    internal static TimeSyncState TimeSyncState = new("BtcTurk One Api");
    #endregion

    internal BtcTurkClientSingleApi(Log log, BtcTurkClient baseClient, BtcTurkClientOptions options) : base(log, options, options.SingleApiOptions)
    {
